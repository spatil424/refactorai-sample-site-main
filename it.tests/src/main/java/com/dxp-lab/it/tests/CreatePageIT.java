...
import com.adobe.cq.cloud.testing.it.smoke.rules.AuthorClassRule;
import com.adobe.cq.cloud.testing.it.smoke.rules.WcmRule;
import com.adobe.cq.cloud.testing.util.CQClient;
...
public class CreatePageIT {
    ...
    // The AuthorClassRule represents an author service. The rule will read
    // the hostname and port of the author service from the system properties
    // passed to the tests.

    @ClassRule
    public static final AuthorClassRule authorClassRule = new AuthorClassRule();

    // WcmRule decorates your test and adds additional functionality on top of
    // it, like session stickyness, test filtering and identification of the
    // test on the remote service.

    @Rule
    public WcmRule wcmRule = new WcmRule(authorClassRule.authorRule);
    ...
    @Rule
    public com.adobe.cq.cloud.testing.it.smoke.rules.Page root = new com.adobe.cq.cloud.testing.it.smoke.rules.Page(authorClassRule.authorRule);

    static CQClient adminAuthor;

    // Thanks to the AuthorClassRule, we can create a CQClient bound to the
    // admin user on the author instance.

    @BeforeClass
    public static void beforeClass() {
        adminAuthor = authorClassRule.authorRule.getAdminClient(CQClient.class);
    }
    ...
    public void testCreatePage() throws InterruptedException {
        ...
        com.adobe.cq.cloud.testing.util.CQAssert.assertCQPageExistsWithTimeout(adminAuthor, root.getPath(), TIMEOUT, 500);
    }
